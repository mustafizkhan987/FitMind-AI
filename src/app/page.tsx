...
export default function Home() {
  const { name, waterDrank, workouts, caloriesConsumed, sleepLogs } = useStore();

  const totalSleepHours = sleepLogs.length > 0 ? sleepLogs[0].hours : 0;
  const totalSleepMins = sleepLogs.length > 0 ? (sleepLogs[0].hours % 1) * 60 : 0;
  ...
  return (
    ...
    >
      <motion.div variants={item} className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight text-white mb-2">
            Welcome back, <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">{name}</span>
          </h1>
          <p className="text-zinc-400 font-medium">Here&apos;s your health summary for today.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800/50 shadow-inner shadow-white/5 rounded-xl px-5 py-2.5 flex items-center gap-2">
      ...
      <motion.div variants={container} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <StatCard
          title="Hydration"
          value={(waterDrank * 0.25).toString()}
          unit="L / 2.0L"
          icon={<Droplets className="w-5 h-5 text-blue-400" />}
          trend={waterDrank >= 8 ? "Goal Met!" : `${8 - waterDrank} to go`}
          trendUp={waterDrank >= 8}
          glowColor="group-hover:shadow-blue-500/10"
        />
        <StatCard
          title="Sleep"
          value={sleepLogs.length > 0 ? `${Math.floor(totalSleepHours)}h ${Math.round(totalSleepMins)}m` : "0h 0m"}
          unit="last night"
          icon={<Moon className="w-5 h-5 text-indigo-400" />}
          trend={sleepLogs.length > 0 ? "Logged" : "No Data"}
          trendUp={sleepLogs.length > 0}
      ...
      <motion.div variants={container} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={item} className="lg:col-span-2 bg-zinc-900/40 backdrop-blur-sm border border-zinc-800/60 rounded-3xl p-6 h-[400px] flex flex-col relative overflow-hidden group hover:border-zinc-700/50 transition-colors duration-300">
          <div className="absolute inset-0 bg-gradient-to-b from-white/[0.02] to-transparent pointer-events-none" />
          <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
            Activity Overview
          </h2>
          <div className="flex-1 flex items-center justify-center border-2 border-dashed border-zinc-800/60 rounded-2xl bg-zinc-950/20 group-hover:border-zinc-700 transition-colors duration-300">
            <p className="text-zinc-500 font-medium">Interactive Chart will render here</p>
          </div>
        </motion.div>

        {/* AI Health Twin Mini-Card */}
        <motion.div variants={item} className="bg-gradient-to-b from-zinc-900 to-zinc-950 border border-zinc-800 rounded-3xl p-6 relative overflow-hidden group hover:border-emerald-500/30 transition-colors duration-300 shadow-2xl">
          <div className="absolute -top-24 -right-24 w-64 h-64 bg-emerald-500/10 blur-[30px] opacity-30 rounded-full group-hover:bg-emerald-500/20 transition-all duration-300 pointer-events-none" />
          ...
          <div className="space-y-5 relative z-10">
            ...
            <div className="bg-zinc-950/50 border border-zinc-800/80 p-5 rounded-2xl backdrop-blur-sm group-hover:border-emerald-500/20 transition-colors duration-300">
...
function StatCard({ title, value, unit, icon, trend, trendUp, glowColor }: any) {
  return (
    <motion.div variants={item} className={`group bg-zinc-900/40 backdrop-blur-sm border border-zinc-800/60 rounded-3xl p-6 hover:border-zinc-700/80 transition-all duration-300 hover:shadow-xl ${glowColor} relative overflow-hidden`}>
      <div className="absolute inset-0 bg-gradient-to-br from-white/[0.02] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      <div className="flex items-center justify-between mb-6 relative z-10">
        <div className="w-12 h-12 rounded-2xl bg-zinc-950 flex items-center justify-center border border-zinc-800/50 shadow-inner group-hover:scale-110 transition-transform duration-300">
          {icon}
        </div>
